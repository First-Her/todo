const nameTextInput = document.getElementById("inputName") as HTMLInputElement | null;
const telephoneInput = document.getElementById("inputTelephone") as HTMLInputElement | null;
const jobSelect = document.getElementById("mySelect") as HTMLSelectElement | null;
const btnCreate = document.getElementById("createBtn") as HTMLButtonElement | null;
const containerMainBlock = document.getElementById("main-container") as HTMLButtonElement | null;
...
function checkingField() {
    if (btnCreate instanceof HTMLButtonElement) {
        btnCreate.disabled = !(
            cardName.length > 0 && cardPhone.length === 11 && cardSelect.length > 0
        );
    } else {
...
async function cardCreate(user) {
    try {
...
async function cardChange(id: string, user: { name: string; phone: string; jobPosition: string }) {
    try {
        const response = await fetch(`http://localhost:8080/task/${id}`, {
            method: "PUT",
            headers: {
...
function renders() {
    ...
    let id = 0;
    cardData.forEach((item: cardData) => {
